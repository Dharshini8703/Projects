...
import AgentLogout from './logout';
    ...
    const handleSubmit = async(e) => {
        ...
        if (!validate) {
            ...
            try {
                ...
                setAgentId(res.data.agent.agent_id);
                setName(res.data.agent.name);
                setPhoneNumber(res.data.agent.phone);
                setEmail(res.data.agent.email);
                setUsername(res.data.agent.username);
                setCompany(res.data.agent.company);
                setPosition(res.data.agent.position);
                const imageUrl = `http://localhost:3000/agentImages/${res.data.agent.filename}`;
                setImage(imageUrl);
                setValidate(true);
            }
            catch(error) {
            ...
            <div className = "login-box">
                {!validate ? (
                    ...
                    <div className='admin-profile'>
                        <h1 className="my-heading">agent Profile</h1>
                        ...
                        <div className='profile-table'>
                            <table>
                                <tr>
                                    <td className='head'>agent ID</td>
                                    <td className='data'>{agentId}</td>
                                </tr>
                                <tr>
