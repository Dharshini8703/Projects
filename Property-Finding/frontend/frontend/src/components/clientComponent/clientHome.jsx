...
function ClientHome() {
    ...
    const handleSubmit = async(e) => {
        ...
        if (!validate) {
            ...
            try {
                const res = await axios.get('http://localhost:3000/api/clients/clientDetails', {
                    headers: {
                        'x-auth-token': token 
                    }
                });
                setClientId(res.data.client.client_id);
                setName(res.data.client.name);
                setPhoneNumber(res.data.client.phone);
                setEmail(res.data.client.email);
                setUsername(res.data.client.username);
                setAddress(res.data.client.address)
                const imageUrl = `http://localhost:3000/clientImages/${res.data.client.profile_name}`;
                setImage(imageUrl);
                setValidate(true);
            }
            catch(error) {
            ...
            <div className = "login-box">
                {!validate ? (
                    ...
                    <div className='admin-profile'>
                        <h1 className="my-heading">Client Profile</h1>
                        ...
                        <div className='profile-table'>
                            <table>
                                <tr>
                                    <td className='head'>Client ID</td>
                                    <td className='data'>{clientId}</td>
                                </tr>
                                <tr>
