...
function AdminResetPassword(props) {
    ...
    const handleInputChange = (e) => {
        ...
        let numPattern = /^\d+$/;
        if (name === 'otp') {
            (value.length !== 4 || !numPattern.test(otp)) ? setValid((prevValid) => ({ ...prevValid, otp: false })) : setValid((prevValid) => ({ ...prevValid, otp: true }));
            setOtp(value);
        }
        if (name === 'newPassword') {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post('http://localhost:3000/admin/resetPassword', { otp }, {
                headers: {
            ...
            <div className="login-box1">
                <div className="form-box" id="form-box">
                    ...
                    <div className="input-box">
                        ...
                        {!valid.otp && (
                            <div>
                            <span>Please enter 4 digit otp</span><br />
                            </div>
                        )}
